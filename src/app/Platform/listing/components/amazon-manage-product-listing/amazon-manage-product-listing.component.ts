...
import { AmazonCategorySpecificationsDialogComponent } from '../amazon-category-specifications-dialog/amazon-category-specifications-dialog.component';
...
@Component({
  selector: 'app-amazon-manage-product-listing',
  templateUrl: './amazon-manage-product-listing.component.html',
  styleUrls: ['./amazon-manage-product-listing.component.scss'],
})
export class AmazonManageProductListingComponent implements OnInit {
  ...
  getUserMarketplacesRegions() {
    this.listingService
      .getUserMarketplacesRegions(this.userData.companyId, Channels.Amazon)
      .subscribe(
        (response) => {
          if (response.body.status == 200) {
  ...
  openeBayCategorySpecificationsSettingsDialogForBulkListing(
    ...
    const dialogRef = this.dialog.open(
      AmazonCategorySpecificationsDialogComponent,
      {
        width: '1000px',
        data: {
  ...
  submitAmazonProductListing() {
    ...
    if (!this.submitted) {
      ...
      this.amazonListingService.listProductsFeedOnAmazon(postData).subscribe(
        (response) => {
          if (response.body.status === 200) {
            this.selectedRows = [];
